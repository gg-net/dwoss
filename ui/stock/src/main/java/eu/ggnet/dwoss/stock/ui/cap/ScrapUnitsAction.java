    ...
    public void actionPerformed(ActionEvent e) {
        saft.build().dialog()
                .eval(() -> {
                    ...
                    dialog.setTitle("Verschrottung");
                    dialog.setHeaderText("(Mehrere) SopoNummer(n) verschrotten.\nBei mehreren SopoNummern, Diese durch Freizeichen oder Kommas trennen.");
                ...
                .thenCompose((Map<String, SimpleStockUnit> m) -> saft.build().title("Verschrottung").fxml().eval(() -> m, ScrapController.class).cf())
                .thenApply((s) -> UiUtil.exceptionRun(() -> remote.lookup(StockApi.class).scrap(s.ids(), s.comment(), guardian.getUsername())))
                .thenAccept(r -> saft.build().title("Ergebnis der Verschrottung").fxml().show(() -> r, ScrapResultController.class))
                .handle(saft.handler());
    }
