 ...
 */
public class CommunicationListCell extends ListCell<Communication> {
    ...
    public CommunicationListCell() {
        hbox = new HBox();
        hbox.setSpacing(5.0);
        hbox.setAlignment(Pos.CENTER);
        hbox.setMinHeight(24.0);

        communicationButton = new RadioButton();
        ...
        //fill the HBox
        hbox.getChildren().addAll(communicationButton, identifer, fillregion, editImg, delImg);
        HBox.setHgrow(fillregion, Priority.ALWAYS);

    }

    @Override
    protected void updateItem(Communication item, boolean empty) {
        ...
        if ( item.isPrefered() ) {
            communicationButton.setSelected(true);
        } else {
            communicationButton.setSelected(false);
        }
        identifer.setText(item.getIdentifier());
