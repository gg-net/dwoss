...
public class ViewReportResult implements Serializable {
    ...
    public EnumMap<Type, NavigableSet<ReportLine>> getRelevantLines() {
        ...
        for (Type keySet : Type.allReportable()) {
            if ( this.lines.get(keySet) == null ) continue;
            copy.put(keySet, this.lines.get(keySet).stream().filter((t) -> t.isAddedToReport()).collect(Collectors.toCollection(() -> new TreeSet<>())));
        }
        return copy;
    }
