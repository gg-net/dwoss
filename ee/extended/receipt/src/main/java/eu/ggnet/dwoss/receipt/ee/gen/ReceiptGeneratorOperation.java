...
public class ReceiptGeneratorOperation {
    ...
    private ProductSpec makeProductSpec(boolean generateCostPrice) {
        ...
        ProductModel model = new ProductModelEmo(specEm).request(
                spec.getModel().getFamily().getSeries().getBrand(),
                spec.getModel().getFamily().getSeries().getGroup(),
                spec.getModel().getFamily().getSeries().getName(),
                spec.getModel().getFamily().getName(),
                spec.getModel().getName());
        specEm.flush();
        
        spec = productProcessor.create(new SpecAndModel(spec, model.getId(), 0, sc, false));
