...
public class AuditReporterOperation implements AuditReporter {
    ...
    public FileJacket onRollIn() {
        ...
        FileJacket fj = createReport(uniqueUnits);
        m.finish();
        return fj;
    }
    ...
    private static List<Integer> toUniqueUnitIds(List<StockTransaction> rollInTransactions) {
    ...
    private static FileJacket createReport(List<UniqueUnit> uniqueUnits) {
        ...
        document.add(sheet);
        return new FileJacket("Audit", ".xls", LucidCalc.createWriter(LucidCalc.Backend.XLS).write(document));
    }
}
